...
def print_levels(remove_duplicates: bool = False) -> None:
    levels = get_all_levels(remove_duplicates)
    sorted_levels = sorted(levels.values(), key=lambda lvl: lvl.level)
    formatted_levels = [
        f"{lvl.color}{lvl.name}[{lvl.level}]{Style.RESET_ALL}"
